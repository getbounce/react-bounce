...
function ProcessorStateProvider<Parameter extends string, State, ProcessorState>({
	...
	const [initialProcessorState, setInitialProcessorState] = useState(controller.initialProcessorState);

	useEffect(() => {
		const handleProcessorStateChange = (state: ProcessorState) => setInitialProcessorState({ ...state });
