...
import { Controller, ParametersValues } from 'foobartestxyz';
...
function createHooks<Parameter extends string, State, ProcessorState>(
	...
	return {
		...
		ControllerProvider: ({ children }) => (
			<ControllerParametersProvider controller={controller} ContextProvider={parametersContext.Provider}>
				<ControllerStateProvider controller={controller} ContextProvider={stateContext.Provider}>
					<ProcessorStateProvider controller={controller} ContextProvider={processorStateContext.Provider}>
						{children}
